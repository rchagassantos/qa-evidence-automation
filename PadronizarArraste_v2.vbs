...
If doc Is Nothing Then
  ...
  WScript.Quit 1
End If
' === Criar tabela personalizada abaixo do logo ===
Dim headerRange, headerTable
Set headerRange = doc.Range(0, 0) ' início do documento

' Criar tabela com 4 linhas e 3 colunas
Set headerTable = doc.Tables.Add(headerRange, 4, 3)

' Preencher linha 1
headerTable.Cell(1, 1).Range.Text = "Projeto: Conexão Digital"
headerTable.Cell(1, 2).Range.Text = "Frente:"
headerTable.Cell(1, 3).Range.Text = "Responsável:"

' Preencher linha 2
headerTable.Cell(2, 1).Range.Text = "Distribuidora:"
headerTable.Cell(2, 2).Range.Text = "Data:"
headerTable.Cell(2, 3).Range.Text = ""

' Preencher linha 3
headerTable.Cell(3, 1).Range.Text = "Produto/Serviço:"
headerTable.Cell(3, 2).Range.Text = "Versão:"
headerTable.Cell(3, 3).Range.Text = "Dispositivo:"

' Mesclar linha 4 para Resultado Esperado
headerTable.Cell(4, 1).Merge headerTable.Cell(4, 3)
headerTable.Cell(4, 1).Range.Text = "Resultado Esperado:"

' Centralizar tabela horizontalmente
headerTable.Rows.Alignment = wdAlignParagraphCenter

' Aplicar negrito
headerTable.Range.Font.Bold = True

' Ajustar largura das colunas (opcional)
headerTable.Columns(1).PreferredWidth = wordApp.CentimetersToPoints(6)
headerTable.Columns(2).PreferredWidth = wordApp.CentimetersToPoints(4)
headerTable.Columns(3).PreferredWidth = wordApp.CentimetersToPoints(6)
...
' === Inserir logo da Neoenergia ===
Dim logoPath, logoShape
logoPath = fso.GetParentFolderName(WScript.ScriptFullName) & "\logo.jpg"

On Error Resume Next
Set logoShape = doc.Shapes.AddPicture(logoPath, False, True, _
    wordApp.CentimetersToPoints(1), wordApp.CentimetersToPoints(1), _
    wordApp.CentimetersToPoints(3), wordApp.CentimetersToPoints(3))
If Not logoShape Is Nothing Then
  logoShape.LockAspectRatio = True
  logoShape.WrapFormat.Type = wdWrapTopBottom
  logoShape.RelativeHorizontalPosition = wdRelHorizPosMargin
  logoShape.Left = wordApp.CentimetersToPoints(1)
  logoShape.Top = wordApp.CentimetersToPoints(1)
End If
On Error GoTo 0
